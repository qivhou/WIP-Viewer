...
class JiraSpider:
    ...
    def getCount(self, filter):
        ...
        if self.isLogin :
            ...
            soup = BeautifulSoup(content)
            find_tag =  soup.find(attrs={'class' : 'results-count-total'}) 
            if find_tag != None:
                ...
                #Sql query issues found
                find_tag = soup.find(attrs={'id' : 'jqlerror'})
                msg = filter
                if find_tag!= None:
                    pattern = re.compile(r'<p>(.*)</p>')
                    matchlist = pattern.search(str(find_tag))
                    if matchlist:
                        msg = matchlist.group(1)
                else:
                    #No matching issues found
                    find_tag = soup.find(attrs={'class':'jqlerror-container'})
                    pattern = re.compile(r'<p>(.*)</p>')
                    matchlist = pattern.search(str(find_tag))
                    if matchlist:
                        msg = matchlist.group(1)         
                ret = 0
                logging.info("Filter Error : %s" % msg)
        else :
            ret = 0
...
if __name__ == '__main__':       
    jiratest = JiraSpider()
    jiratest.connect()
    print jiratest.getCount('project=Watt and fixversion >= "v05.00.05"  AND  status = "open"')
    #print jiratest.getCount('project in (SEL)')
    print jiratest.getCount('project=Watt and fixversion >= "v05.00.05"  AND  status = "blocked"')